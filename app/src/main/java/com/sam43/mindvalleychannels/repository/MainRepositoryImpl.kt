...
class MainRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChannelsData(): Flow<EventState<List<ChannelsIncludingCourseAndSeries>>> =
        bindDataFromServer(
            ...
            saveNetworkResponse = {
                db.channelsDao.insertChannelsWithCoursesAndSeries(it.toEntity())
            }
        )
    ...
    override suspend fun getCategoriesData(): Flow<EventState<List<CategoryEntity>>> =
        bindDataFromServer(
            ...
            saveNetworkResponse = {
                db.categoryDao.insertCategories(it.toEntity()) }
        )
    ...
    override suspend fun getNewEpisodesData(): Flow<EventState<List<EpisodeEntity>>> =
        bindDataFromServer(
            ...
            saveNetworkResponse = { db.episodeDao.insertEpisodes(it.toEntity()) }
        )
}